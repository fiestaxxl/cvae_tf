...
def plot_mols(file_path, save_path):
    ...
    img=Draw.MolsToGridImage(smiles,molsPerRow=4,subImgSize=(700,700),returnPNG=False)
    filename = os.path.join(save_path, os.path.splitext(os.path.basename(file_path))[0], '.png')
    img.save(filename)

def plot_mol(smiles, save_path):
    mol = Chem.MolFromSmiles(smiles)
    img = Draw.MolToImage(mol, returnPNG=False)
    filename = os.path.join(save_path, smiles, '.png')
    img.save(filename)
